![AtmoLLM logo](assets/AtmoLLM_Logo.png)
...
**AtmoLLM** is a fork of **AirLLM** by Gavin Li. It optimizes inference memory usage, allowing 70B large language models to run inference on a single 4GB GPU card without quantization, distillation and pruning. And you can run **405B Llama3.1** on **8GB vram** now.
...
[![PyPI - AtmoLLM](https://img.shields.io/pypi/format/atmollm?logo=pypi&color=3571a3)
](https://pypi.org/project/atmollm/)
...
First, install the AtmoLLM pip package.
...
Then, initialize AtmoLLM, pass in the huggingface repo ID of the model being used, or the local path, and inference can be performed similar to a regular transformer model.
...
Compatibility note: existing code that imports `airllm` continues to work in this fork.
...
If you find
If AtmoLLM is useful in your research and you wish to cite it, please use the following original AirLLM citation and mention this fork:
BibTex entry:
